...
def ship_event(selected_ship):
    ...
    if "Fast and lightweight" in selected_ship["Stats"]:
        ...
        if event_chance <= 3:  # 30% chance of encountering an event
            ...
            print(center_text(f"\n{selected_ship['name']} had a smooth expedition."))
        ...
        if event_chance <= 2:  # 20% chance of encountering an event
            print(center_text(f"{selected_ship['name']} encountered a hostile alien race. Its heavy armor absorbed the damage, but it's returning with battle scars."))
...
def send_ship_on_expedition(inventory: Inventory):
    ...
    for i, ship_name in enumerate(inventory.fleet, start=1):
        print(center_text(f"{i}. {ship_name}"))
    try:
        ship_choice = int(input(center_text("Enter the number of the ship you want to send: ")))
    except ValueError:
        ...
        expedition_duration = int(input(center_text("Enter the duration of the expedition in minutes: ")))
        ...
        print(
            center_text(f"{selected_ship['name']} has returned from the expedition!\n"
